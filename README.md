...
- **`src/engine/`** — the simulation core. Pure logic, no DOM access, no
  ...
  - `replay.ts` — replay/export of a run (#33), built on top of #29's
    ...
    grid snapshots: a `Replay` is `{ initialState, inputs, endTick }`,
    where `inputs` is a `RecordedInput[]` — every add-creature click and
    settings change that happened after `initialState` was captured, each
    tagged with the tick already completed when it was applied — and
    `endTick` is the tick the original recording stopped at. Replaying
    ...
    `Simulation.rng` stream, consumed in the same order it was live —
    until `tickCount` reaches `endTick`, where playback stops rather than
    continuing on into ticks the recording never spoke for. Far smaller
    than storing the grid every tick, at the cost of needing to
...
- **`src/worker/`** — the `Simulation` (grid, settings, RNG, tick loop) runs
  ...
  `initialState`, queues `inputs` sorted by tick, and remembers the
  replay's `endTick`, then the normal tick loop (`loop()`/`stepOnce`) calls
  `applyDueReplayInputs()` after every `step()` to apply whatever's now
  due — reusing the existing pause/speed/step controls as the replay's
  playback controls, rather than a separate fast-forward path. The instant
  `tickCount` reaches `endTick`, that same function sets `paused = true`
  (breaking out of `loop()`'s own catch-up batch immediately, so a single
  call ticking through several backlogged ticks at once can't overshoot
  it either) and posts `replayFinished`, so playback stops exactly where
  the recording did instead of continuing into ticks it never spoke for —
  the main thread mirrors that pause into its own button state on
  `replayFinished` rather than re-requesting it. A wholesale replacement
  (`importState`/`applyPreset`) cancels any in-progress recording, since
  its `initialState` would no longer describe the running simulation.
...
with the tick it happened at, plus the tick the recording stopped at
(`endTick`). Loading one re-runs the simulation from that seed and
re-applies the same interactions at the same ticks, reproducing the
original run byte-for-byte (`test/engine/replay.test.ts`) — this only
...
Playback stops automatically the moment the simulation reaches `endTick` —
continuing past it would just be fresh, unrecorded randomness rather than
anything the replay actually speaks for, so the worker pauses itself there
(`test/worker/simulation-worker.test.ts` proves this holds even across a
single real-time tick batch that would otherwise blow straight through it).
From that point the loaded run behaves like any other paused simulation:
