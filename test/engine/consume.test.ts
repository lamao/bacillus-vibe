...
describe('consume (phase 4)', () => {
  it('grants SunYield to Sun-consumers regardless of movement or position', () => {
    const settings = testSettings({ sunYield: 25 });
    const grid = emptyGrid(settings);
    const mover = organic({ x: 1, y: 1 }, { dna: dna({ consume: 'Sun', canMove: true }), energy: 100, size: 1000 });
    const stationary = organic({ x: 8, y: 8 }, { dna: dna({ consume: 'Sun', canMove: false }), energy: 100, size: 1000 });
  ...
  it('caps Sun gain at maxSize, growing size when energy is already full', () => {
    ...
    const grid = emptyGrid(settings);
    const o = organic({ x: 1, y: 1 }, { dna: dna({ consume: 'Sun' }), energy: 1000, size: 1000 });
    place(grid, o);
    consume(grid, settings);
  ...
  it('does nothing for a mover with non-Sun consume (no passive digestion)', () => {
    const settings = testSettings();
    const grid = emptyGrid(settings);
    const mover = organic({ x: 5, y: 5 }, { dna: dna({ consume: 'Green', canMove: true }), energy: 500, size: 1000 });
  ...
  it('drains matching minerals within ConsumingRange for a non-mover, splitting gain/waste', () => {
    const settings = testSettings({ consumingRange: 2, mineralsYield: 10, productionPerformance: 0.1 });
    const grid = emptyGrid(settings);
    const o = organic({ x: 5, y: 5 }, { dna: dna({ consume: 'Green', canMove: false }), energy: 500, size: 1000 });
  ...
  it('drains matching prey organics, clamping their energy to their reduced size', () => {
    ...
    const grid = emptyGrid(settings);
    const predator = organic({ x: 5, y: 5 }, { dna: dna({ consume: 'Blue', canMove: false }), energy: 0, size: 1000 });
